...
*Shipped so far (2026-09-09):* the dashboard is always the launch screen (with a Settings pin to override into one group); the currency-bucketed header (`DashboardTotals` in ClanTabKit + `DashboardTotalsHeader`); and the **worker half** of the push-carried balance — `notifyGroup` now takes a `recipientBalance: { currency, balances }` context and, per recipient, folds `{ balanceCurrency, balanceNetMinor }` (that member's own net in the mutation's currency, `"0"` when they hold no nonzero balance in it) into `data`. `GroupDO.claimedIdentitiesExcluding` became `claimedRecipientsExcluding`, returning `{ sub, memberId }[]` — the one extra field needed, on the fan-out call that already happens. Still open: the iOS receipt handler that reads those keys into the cache, and the bounded fallback sync. **Update this note** the moment any further piece ships or the design changes.
