...
Response: 200 {
  ...
  expenses: [{ id, payerId, amountMinor, currency, description, date, splitType,
               splits: [...], items?, category?, categoryIcon? }],
...
*Offline behaviour (investigated 2026-09-09, `CHECKLIST.md` "Offline queueing for adding an expense").* `URLSessionTransport` is backed by `URLSession.shared`, whose `waitsForConnectivity` is `false`, so a mutation attempted with no connectivity **fails fast** (`URLError.notConnectedToInternet`) rather than hanging; a mid-request drop is bounded by the 60s request timeout. `AddExpenseView.save()` catches the error, surfaces it, and leaves the sheet open with the form intact — retry works once back online. The backend is already replay-safe: `GroupDO.addExpense`/`addSettlement` are idempotent on the client-supplied `id`. What a real offline queue would cost is not the network layer but **optimistic display** — showing a not-yet-synced expense in the feed and in balances, which this section's "no optimistic UI in v1" rule deliberately rules out — plus the edge cases (member deleted server-side, group left, account deleted). Estimated at ~100–140k for the full feature, ~25–30k for a single-slot "retry the last failed add" that needs no feed/balance changes. Not built; revisit only on a signal that people add expenses offline. `ClanTabKit.UserDefaultsIdentityStore` reads/writes `UserDefaults` under the key `"clantab:" + groupId` to remember `{ memberId, displayName }` per device per group — the iOS equivalent of a web client's `localStorage`-backed `identity.ts`.
...
**Local device cache — what persists, what doesn't, and why (2026-09-09).** `KnownGroupsStore` (`UserDefaults`) is the only *cross-launch* local cache: per known group its `UserDefaults` blob holds display fields only — `{ name, emoji, myBalances, lastOpenedAt }` — never the expense/settlement line items themselves. The group's `accessToken` is a credential and lives in the **Keychain** instead (`GroupAccessTokenStoring` / `KeychainGroupAccessTokenStore`, one generic-password item holding a `[groupId: token]` map, `kSecAttrAccessibleAfterFirstUnlock` — same construction as `KeychainSessionStore`); `UserDefaultsKnownGroupsStore.all()` merges it back onto each `KnownGroup` on read, and a one-time `init` migration moves any token still embedded in an older blob across (§8 — this closed the gap that review found). `GroupViewModel.state` (the full ledger for whichever group is on screen) lives only in that view's in-memory `@Observable` state — gone the moment the app terminates, refetched fresh on the next launch. Deliberate, not an oversight: it keeps plaintext ledger detail off disk except in the two intentional off-device copies already covered above (the CSV/JSON export share sheet, and the tier-2 CloudKit backup — both user-initiated or opt-out-able). `myBalances` is a display cache exactly like the widget snapshot (`WidgetSnapshotStore`'s own doc comment) — never a second source of truth, always replaced wholesale by the same `Balances.compute` output every other view uses. See §8 for a gap found in this same review: `accessToken` in that same `UserDefaults` blob doesn't get the Keychain protection the session token gets.

**Group dashboard (2026-09-09 brainstorm; build tracked in `CHECKLIST.md`, shipped bar cosmetics).** The launch screen becomes a cross-group dashboard (every known group + a currency-bucketed net-balance summary) instead of auto-opening the device's one group. Naively refreshing every known group's balance on every cold launch would fan one `GroupDO` read out per group on the single highest-frequency event in the app — considered and rejected, §12. The chosen design treats **writes as the trigger, not launches**: `notifyGroup`'s existing per-recipient push loop (`worker/src/lib/notify.ts`) already has that recipient's freshly-mutated balance in hand from the write it's notifying about, so attaching that recipient's own net to the push payload's `data` field costs nothing new — the iOS push handler writes it straight into `KnownGroupsStore.updateBalances()` on receipt (foreground or background), and the dashboard reads only that local cache, zero network calls on open. Push delivery isn't guaranteed (iOS throttles/drops silently; some users deny notification permission outright), so this needs a bounded fallback, not blind trust: pull-to-refresh, plus a time-boxed periodic reconcile (once a day, or once per cold-start-after-N-hours — never every launch) that does the rare N+1 catch-up fan-out, the same shape `GET /api/auth/people` (§13) already uses for cross-group settling — probably its own sibling worker endpoint rather than N client round-trips.

*Shipped so far (2026-09-09):* the dashboard is always the launch screen (with a Settings pin to override into one group); the currency-bucketed header (`DashboardTotals` in ClanTabKit + `DashboardTotalsHeader`); and the full push-carried balance path. **Worker:** `notifyGroup` takes a `recipientBalance: { currency, balances }` context and, per recipient, folds `{ balanceCurrency, balanceNetMinor }` (that member's own net in the mutation's currency, `"0"` when they hold no nonzero balance in it) into `data`; `GroupDO.claimedIdentitiesExcluding` became `claimedRecipientsExcluding` returning `{ sub, memberId }[]`; `sendPush` now sends a combined alert + `content-available` payload. **iOS:** `AppDelegate.applyCarriedBalance(from:)` folds those keys into the group's cached `myBalances` (`Balances.applyingCarriedBalance` — per-currency merge, dropped at zero) via `KnownGroupsStore.updateBalances`, from `willPresent` (foreground), `didReceive` (tap) and `didReceiveRemoteNotification` (background wake); `UIBackgroundModes: [remote-notification]` added. **Fallback sync (also shipped):** `GET /api/auth/groups/balances` (§13) — a concurrent fan-out of the caller's own per-group balance, same shape as `/people`. `AuthViewModel.reconcileGroupBalances(force:)` folds it into `KnownGroupsStore`, gated by `DashboardReconcile.shouldReconcile` (`UserDefaultsDashboardSyncStore` timestamp, 6h staleness) on launch and unconditionally on `StartView` pull-to-refresh. What's left is cosmetic only (the parked cross-group spend graphs, §12). **Update this note** if any of it changes.
...
- ~~**Client-side storage: the group `accessToken` doesn't get the same protection as the session token — a real gap, found 2026-09-09.**~~ **Fixed 2026-09-09.** `accessToken` is exactly the class of secret `AGENTS.md` calls "the underlying data-access credential" — the same standing as the session token — so it now gets the same protection: `KeychainGroupAccessTokenStore` (`GroupAccessTokenStoring`), one `kSecClassGenericPassword` item holding a `[groupId: token]` JSON map, `kSecAttrAccessibleAfterFirstUnlock`, mirroring `KeychainSessionStore`. `UserDefaultsKnownGroupsStore` no longer encodes `accessToken` in its `UserDefaults` blob (`KnownGroup.CodingKeys` omits it); it writes the token through to the Keychain store on `remember`, deletes it on `forget`, and merges it back onto each `KnownGroup` on `all()`. A one-time `init` migration lifts any token still inline in a pre-existing blob into the Keychain and rewrites the blob clean. `myBalances`/`name`/`emoji` stay in `UserDefaults` — display data, not credentials.
