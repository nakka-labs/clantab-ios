...
/// Abstracts local persistence of per-group identity so SquareKit's callers never
...
/// `UserDefaults`-backed identity store, namespaced `"squarely:<groupId>"` — the
/// same key scheme as the web client's `localStorage["squarely:" + groupId]`.
