...
Each group has a long, unguessable link and a short join code. **Anyone who has
the link or code can view and edit that group's records** — that is the only
access control, by design (the same model as a shareable document link). Don't
post a group's link or code publicly.
...
Because there are no accounts, there is no "delete my account." A group's records
persist on the backend so other members keep their history. To have a specific
group's data deleted, contact us (below) with the group's join code — we will
remove that group's records.
...
where practical, a note will appear in the app.
